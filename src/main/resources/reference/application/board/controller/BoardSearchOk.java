...
public class BoardSearchOk implements Action {
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		// 게시글의 전체 갯수
		int total = dao.getSearch(boardTitle);
		...
		//boarMap에다가 key값과 value값을 넣어준다.
		dto.setStartRow(startRow);
		dto.setRowCount(rowCount);
		dto.setBoardTitle(boardTitle);
		
		req.setAttribute("boardList", dao.search(dto));
		req.setAttribute("page", page);
		req.setAttribute("startPage", startPage);
		req.setAttribute("endPage", endPage);
		req.setAttribute("realEndPage", realEndPage);
		req.setAttribute("total", total);
		req.setAttribute("search", boardTitle);

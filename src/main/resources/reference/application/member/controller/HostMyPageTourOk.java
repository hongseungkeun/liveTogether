...
public class HostMyPageTourOk implements Action{
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		// 게시글의 전체 갯수
				...
				for (int i = 0; i < dao.selectGuestAll(roomMap).size(); i++) {
					RoomGuestDTO dto = new RoomGuestDTO();
					dto.setHouseNumber(dao.selectRoomAll(roomMap).get(i).getHouseNumber());
					dto.setRoomName(dao.selectRoomAll(roomMap).get(i).getRoomName());
					dto.setRoomType(dao.selectRoomAll(roomMap).get(i).getRoomType());
					dto.setRoomGender(dao.selectRoomAll(roomMap).get(i).getRoomGender());
					dto.setStatus(dao.selectRoomAll(roomMap).get(i).getStatus());
					dto.setMemberName(dao.selectGuestAll(roomMap).get(i).getMemberName());
					dto.setMemberGender(dao.selectGuestAll(roomMap).get(i).getMemberGender());
					dto.setMemberPhone(dao.selectGuestAll(roomMap).get(i).getMemberPhone());
					alDTO.add(dto);
				}
				...
				af.setPath("/member/hostmypagetour.jsp");

...
public class HostMemberListOk implements Action {
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		// 게시글의 전체 갯수
		int total = dao.getRoomTotalFour();
		...
		for (int i = 0; i < dao.selectGuestAllFour(roomMap).size(); i++) {
			RoomGuestDTO dto = new RoomGuestDTO();
			dto.setHouseNumber(dao.selectRoomAllFour(roomMap).get(i).getHouseNumber());
			dto.setRoomName(dao.selectRoomAllFour(roomMap).get(i).getRoomName());
			dto.setRoomType(dao.selectRoomAllFour(roomMap).get(i).getRoomType());
			dto.setRoomGender(dao.selectRoomAllFour(roomMap).get(i).getRoomGender());
			dto.setStatus(dao.selectRoomAllFour(roomMap).get(i).getStatus());
			dto.setMemberName(dao.selectGuestAllFour(roomMap).get(i).getMemberName());
			dto.setMemberGender(dao.selectGuestAllFour(roomMap).get(i).getMemberGender());
			dto.setMemberPhone(dao.selectGuestAllFour(roomMap).get(i).getMemberPhone());
			alDTO.add(dto);
		}
		...
		af.setPath("/member/hostmemberlist.jsp");

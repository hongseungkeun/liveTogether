...
public class HostMyPageReadyOk implements Action{
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		for (int i = 0; i < dao.selectGuestAllTwo(roomMap).size(); i++) {
			RoomGuestDTO dto = new RoomGuestDTO();
			dto.setHouseNumber(dao.selectRoomAllTwo(roomMap).get(i).getHouseNumber());
			dto.setRoomName(dao.selectRoomAllTwo(roomMap).get(i).getRoomName());
			dto.setRoomType(dao.selectRoomAllTwo(roomMap).get(i).getRoomType());
			dto.setRoomGender(dao.selectRoomAllTwo(roomMap).get(i).getRoomGender());
			dto.setStatus(dao.selectRoomAllTwo(roomMap).get(i).getStatus());
			dto.setMemberName(dao.selectGuestAllTwo(roomMap).get(i).getMemberName());
			dto.setMemberGender(dao.selectGuestAllTwo(roomMap).get(i).getMemberGender());
			dto.setMemberPhone(dao.selectGuestAllTwo(roomMap).get(i).getMemberPhone());
			alDTO.add(dto);
		}
		...
		af.setPath("/member/hostmypageready.jsp");

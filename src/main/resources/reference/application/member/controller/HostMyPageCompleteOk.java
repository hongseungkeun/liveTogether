...
public class HostMyPageCompleteOk implements Action{
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		// 게시글의 전체 갯수
		int total = dao.getRoomTotalThree();
		...
		for (int i = 0; i < dao.selectGuestAllThree(roomMap).size(); i++) {
			RoomGuestDTO dto = new RoomGuestDTO();
			dto.setHouseNumber(dao.selectRoomAllThree(roomMap).get(i).getHouseNumber());
			dto.setRoomName(dao.selectRoomAllThree(roomMap).get(i).getRoomName());
			dto.setRoomType(dao.selectRoomAllThree(roomMap).get(i).getRoomType());
			dto.setRoomGender(dao.selectRoomAllThree(roomMap).get(i).getRoomGender());
			dto.setStatus(dao.selectRoomAllThree(roomMap).get(i).getStatus());
			dto.setMemberName(dao.selectGuestAllThree(roomMap).get(i).getMemberName());
			dto.setMemberGender(dao.selectGuestAllThree(roomMap).get(i).getMemberGender());
			dto.setMemberPhone(dao.selectGuestAllThree(roomMap).get(i).getMemberPhone());
			alDTO.add(dto);
		}
		...
		af.setPath("/member/hostmypagecomplete.jsp");
